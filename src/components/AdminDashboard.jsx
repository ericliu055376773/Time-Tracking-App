...
const TABS = ['薪資計算', '打卡紀錄', '請假審核', '員工管理', 'WiFi 設定'];
...
export default function AdminDashboard() {
  ...
  async function handleAddEmployee() {
    ...
    if (addForm.role === 'employee') {
      if (!/^\d{10}$/.test(addForm.pin))
        return setAddError('PIN 碼必須是 10 位數字');
    } else {
    ...
    try {
      ...
      const msgs = {
        'auth/email-already-in-use': '此員工編號已被使用',
        'auth/weak-password': 'PIN 碼至少需要 6 碼',
      };
              ...
              <>
                ...
                <label style={labelStyle}>
                  ...
                  <input
                    ...
                    value={addForm.pin}
                    onChange={(e) =>
                      setAddForm((f) => ({ ...f, pin: e.target.value }))
                    }
                    placeholder="••••••••"
                  />
                </label>
            ...
            >
              ...
              <label style={{ ...labelStyle, flex: '1 1 110px' }}>
                ...
                <input
                  ...
                  onChange={(e) => onEditChange('empId', e.target.value)}
                />
              </label>
              <label style={{ ...labelStyle, flex: '1 1 110px' }}>
                <span>薪資類型</span>
...
function WifiSettings() {
  ...
  async function addNetwork() {
    ...
    const newNet = {
      ...
      localSubnet: currentInfo.localIP
        ? currentInfo.localIP.split('.').slice(0, 3).join('.')
        : '',
      ...
      <div className="card" style={{ marginBottom: 16 }}>
          ...
          >
            <div>
              ...
              <span style={{ color: 'var(--amber)' }}>
                {currentInfo.publicIP || '無法取得'}
              </span>
            </div>
            <div>
              本地 IP：
              <span style={{ color: 'var(--amber)' }}>
                {currentInfo.localIP || '無法取得'}
              </span>
            </div>
            <div>
              子網路：
              <span style={{ color: 'var(--amber)' }}>
                {currentInfo.localIP
                  ? currentInfo.localIP.split('.').slice(0, 3).join('.') + '.x'
                  : '無法取得'}
              </span>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <input
            ...
            placeholder="WiFi 名稱（例如：辦公室）"
            style={{ flex: 1, fontSize: 13 }}
          />
          <button
            onClick={addNetwork}
            disabled={!newName.trim() || !currentInfo}
            style={{
              padding: '0 18px',
              background: 'var(--amber)',
              color: '#000',
          ...
          networks.map((n, i) => (
            <div
              ...
              style={{
                ...
                padding: '12px 0',
                borderBottom:
                  i < networks.length - 1 ? '1px solid var(--border)' : 'none',
              }}
            >
              <div>
