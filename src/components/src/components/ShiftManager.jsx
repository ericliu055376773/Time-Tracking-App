            ...
            <div key={shift.id} className="card" style={{ padding: '16px 20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={() => setDeleteConfirm(shift)} style={{
                    padding: '7px 14px', background: 'var(--red-glow)',
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}
          ...
          <div className="card fade-in" style={{ width: '100%', maxWidth: 420, margin: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <label style={lbl}>
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      setForm(f => ({ ...f, id: v, name: v ? v + ' 班' : f.name }));
                    }}
                    placeholder="A"
                    disabled={!!editingId}
              ...
              {/* 工時預覽 */}
              {form.start && form.end && (
                <div style={{ background: 'var(--amberBg, var(--amber-glow))', border: '1px solid rgba(245,158,11,0.25)', borderRadius: 8, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>工作時數</span>
                  <span style={{ fontFamily: 'var(--mono)', fontSize: 16, fontWeight: 700, color: 'var(--amber)' }}>
                    {calcHours(form.start, form.end)}
                  </span>
                </div>
              )}
